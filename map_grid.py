...
class MapGrid:
    ...
    def __init__(self, polygons, node_spacing):
        self.polygons = polygons
        self.node_spacing = node_spacing
        
    def get_mapgrid_dict(self) -> dict:
        
        all_triangles = []
        for polygon in self.polygons:
            triangles = self._split_polygon_into_triangles(np.array(polygon))
        ...
        corners = self.polygons.pop(0)
        
        # Node spacing is the quality of the pathfinding grid
        node_spacing = 50
        map_grid, valid_nodes = self._find_valid_nodes(corners, node_spacing, self.polygons)    
        ...
        return self._grid_to_dict(map_grid, 1)

    # helper functions
    def _split_polygon_into_triangles(self, polygon: list[tuple]) -> list[tuple]:
    ...
    def _find_valid_nodes(self, corners: list[tuple], node_spacing: int, polygons: list[list[tuple]]) -> np.ndarray:
    ...
    def _grid_to_dict(self, grid: np.ndarray, node_spacing: int) -> dict:
